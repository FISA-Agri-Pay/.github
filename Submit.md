...
* **데이터 · 운영 도구** : PostgreSQL Patroni Write/Read 이중화, TrueNAS PV 스토리지, GitLab·Jenkins 소스·빌드.
...
* 코드 링크(스크립트 링크) :
  * `ai-prediction-model/src/models/gru/train_service.py` (서비스별 GRU 학습)
  * `ai-prediction-model/src/evaluation/service_pod_policy.py` (우선순위 기반 Pod 배분 정책)
  * `ai-prediction-model/src/evaluation/pod_policy.py` (필요 Pod 산정)
  * `ai-prediction-model/src/optimization/autoscaling_score.py` (Under-provisioning 기준 모델 평가)
  * `ai-prediction-model/experiments/service_autoscaling/` (Reactive vs Predictive 비교 실험)
  * `ai-prediction-model/docs/onprem-bnpl-autoscaling-design.md` (설계 문서)
...
* 코드 링크(스크립트 링크) :
  * `back-end/service-admin/Dockerfile`
  * `back-end/service-auth/Dockerfile`
  * `back-end/service-batch/Dockerfile`
  * `back-end/service-catalog/Dockerfile`
  * `back-end/service-core/Dockerfile`
  * `back-end/service-payment/Dockerfile`
  * `mcp-aiops-backend/Dockerfile`
  * `back-end/blob/dev/service-admin/Dockerfile`
  * `back-end/blob/dev/service-admin/Jenkinsfile`